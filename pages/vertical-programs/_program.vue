...
export default class extends Vue {
  ...
  get period() {
    return `${this.getTimeSlug(this.program.start, true)} ~ ${this.getTimeSlug(this.program.end, true)}`
  }

  get length() {
    const start = this.program.start instanceof Date ? this.program.start : (new Date(this.program.start))
    const end = this.program.end instanceof Date ? this.program.end : (new Date(this.program.end))
  ...
  getTimeSlug(datetime, colon = false) {
    ...
    return `${this.padStartWithZero(time.getHours())}${colon ? ':' : ''}${this.padStartWithZero(time.getMinutes())}`
  }
